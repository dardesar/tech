...
class KycDocumentRepository
{
    ...
    public function get() {
        ...
        $kycDocument->with(['country', 'selfie', 'documents', 'user']);
    ...
    public function store($data) {
        ...
        $kycDocument->documents()->sync(request()->get('documents'));
    ...
    public function moderate($document, $action) {
        ...
        if($action == "approve") {
            ...
            // Notify user
            Mail::to($document->user)->send(new KycApproved($document->user));
            ...
            // Notify user
            Mail::to($document->user)->send(new KycRejected($document->user, $document->rejected_reason));
        }
